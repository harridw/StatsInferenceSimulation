...
If we look at very large population, maybe 10,000 random exponential observations, we might expect to have a mean, standard deviation, and variance that are very close to the theoretical values previously noted. Let's  If we calculate the mean and variance of a large exponential sample population, like 10,000, we would expect this to be fairly close to the theoretical mean and variance.

**Large Sample Mean & Variance (10,000 random exponential observations)** 
```{r, large_sample_data, include = TRUE, echo = FALSE}
set.seed(100)
...
for (i in 1: n1) sum.var <- sum((lrg.sample$rating[i] - sample.mean)^2)
      round(var(lrg.sample$rating), 3)     ## Population Variance
